...
# DAG object.
with models.DAG(
    "composer_twitter_sentiment_predict",
    ...
    preprocessing = DataflowCreatePythonJobOperator(
        ...
        job_name="twitter_test_preprocess_batch_{{ ds_nodash }}",
        options={
            ...
            "input": "{{ dag_run.conf['work_dir'] }}/input_data/test*.csv",
    ...
    predict = DataflowCreatePythonJobOperator(
        task_id="predict_tweets",
        py_file="{{ dag_run.conf['work_dir'] }}/code/predict.py",
        job_name="twitter_predict_batch_{{ ds_nodash }}",
        options={
            "temp_location": "{{ dag_run.conf['work_dir'] }}/beam-temp/{{ ds_nodash }}",
            "setup_file": os.path.join(
                configuration.get("core", "dags_folder"), "twitter-sentiment/setup.py"
            ...
            "work-dir": "{{ dag_run.conf['work_dir'] }}",
            "model-dir": "{{ dag_run.conf['work_dir'] }}/output_data/{{ ds_nodash }}/model",
            "inputs-dir": "{{ dag_run.conf['work_dir'] }}/output_data/{{ ds_nodash }}/test/part*",
            "outputs-dir": "{{ dag_run.conf['work_dir'] }}/output_data/{{ ds_nodash }}/predictions",
        },
        # TODO: change for dynamic parse from requirements.txt
        py_requirements=[
            "apache-beam[gcp]==2.24.0",
